...
def get_train_embeds_output(model, size, defect_type, transform, device):
    # get out put of applied embed (resnet) on data
    test_data = MVTecAD("Data", defect_type, size, transform=transform, mode="train")
...
def evaluate_model(modelname, defect_type, device="cpu", save_plots=False, size=256, show_training_data=True, model=None, train_embed=None, head_layer=8, density=GaussianDensityTorch()):
    # create test dataset
    ...
    if test_data_eval is None or cached_type != defect_type:
        ...
        test_data_eval = MVTecAD("Data", defect_type, size, transform = test_transform, mode="test")
    ...
    if model is None:
        ...
        model = ADModel(pretrained=False, head_layers=head_layers, num_classes=classes)
    ...
    if train_embed is None:
        train_embed = get_train_embeds_output(model, size, defect_type, test_transform, device)
    ...
    if save_plots:
        ...
        if show_training_data:
            ...
            dataloader_train = DataLoader(train_data, batch_size=32,
                        shuffle=True, num_workers=8, collate_fn=merge_cut_paste,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_plots', default=True, type=to_bool,
    ...
    for model_name, data_type in zip(model_names, types):
        ...
        roc_auc = evaluate_model(model_name, data_type, save_plots=args.save_plots, device=device, head_layer=args.head_layer, density=density())
